...
createSpline knotvals controls = CSpline knotvals' controls' $ tdmaSolver mu (V.replicate (V.length d) 2.0) lambda d
	where
		...
		lambda = V.cons 0.0 $ V.zipWith (\x y->x/(x+y)) (V.tail h) (V.init h)
		mu = V.snoc (V.map (\x->1.0-x) lambda) 0.0
		dj hj hj1 yj1 yj yjm1= 6.0/(hj+hj1)*((yj1-yj)/hj1-(yj-yjm1)/hj)
		d = V.cons 0.0 $ V.zipWith5 dj (V.init h) (V.tail h) 
				(V.tail $ V.tail controls') (V.tail $ V.init controls') 
				(V.init $ V.init controls') V.++ V.singleton 0.0
...
tdmaSolver a b c d = V.reverse $ solution $ V.zip (V.cons 0.0 $ V.reverse cvals) (V.reverse dvals)
	where
		cvals = scanlt0 (\priorc (a1,b1,c1)->c1/(b1-priorc*a1)) $ 
					V.zip3 (V.cons 0.0 $ V.init a) (V.init b) c
		dvals = scanlt0 (\priord (a1,b1,c1,d1)->(d1-a1*priord)/(b1-a1*c1)) $ 
					V.zip4 (V.cons 0.0 a) b (V.cons 0.0 cvals) d
