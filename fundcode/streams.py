        ...
        if isinstance(index,slice):
            ...
            if enddate in self.enddates:
                ...
                while not(enddate in self.enddates):
                    ...
                    enddate = enddate - datetime.timedelta(days=1)
                endindex = self.enddates.index(enddate)
            return ReturnStream(self.startdates[startindex:endindex+1],self.enddates[startindex:endindex+1],
                                self.returns[startindex:endindex+1])
        else:
